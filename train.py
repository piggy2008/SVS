...
def train_single(net, inputs, flows, labels, optimizer, curr_iter, teacher):
    ...
    if args['distillation']:
        ...
        distill_loss_t = (loss0_t + loss1_t) / 2 + loss2_t / 2 + loss3_t / 4 + loss4_t / 8 \
                         + loss5_t / 16 + loss6_t / 4 + loss7_t / 8 + loss8_t / 16 + loss9_t / 2
    ...
    if args['distillation']:
        total_loss = total_loss + 0.3 * distill_loss + 0.3 * distill_loss_t
    else:
