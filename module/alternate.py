...
class Alternate2(nn.Module):
    def __init__(self, division=8):
        super(Alternate2, self).__init__()
        self.fuse_conv = nn.Conv2d(64 + division, 64, kernel_size=3, padding=1)
        self.fuse_bn = nn.BatchNorm2d(64)
        self.division = division
    ...
    def forward(self, x, sal):
        if sal.size()[2:] != x.size()[2:]:
            sal = F.interpolate(sal, size=x.size()[2:], mode='bilinear')
