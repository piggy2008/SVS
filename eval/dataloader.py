...
class EvalDataset(data.Dataset):
    def __init__(self, img_root, label_root):
        # lst_label_folders = sorted(os.listdir(label_root))
        lst_pred_folders = sorted(os.listdir(img_root))
        lst = []
        for folder in lst_pred_folders:
            imgs = os.listdir(os.path.join(img_root, folder))
            imgs.sort()
            for img in imgs:
                lst.append(os.path.join(folder, img))
